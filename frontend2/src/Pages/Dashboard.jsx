...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        const res = await axios.get(`${API_BASE}/pages/stats`, {
    ...
    const fetchRecentPages = async () => {
      try {
        const res = await axios.get(`${API_BASE}/pages?limit=5&sort=recent`, {
          withCredentials: true,
        });
        // Map 'id' to '_id' for consistency
        const pages = res.data.map((p) => ({ ...p, _id: p.id }));
        setRecentPages(pages);
      } catch (err) {
        console.error(
          "❌ Failed to fetch recent pages",
