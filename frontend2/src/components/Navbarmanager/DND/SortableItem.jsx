...
const SortableItem = memo(
  ({ item, onEdit, onDelete, level = 0, onToggle, activeMenus, isFirstRoot }) => {
    ...
    return (
      <>
        ...
        >
          <div className="flex items-center justify-between bg-gray-100 p-2 rounded-md group">
            <div className="flex items-center gap-2">
              ...
              <span className={`font-medium ${isActive ? "text-green-600" : "text-gray-700"}`}>
                {item.title}
              </span>
            </div>
        ...
        {/* Custom Menu Toggle (only for the first root item) */}
        {level === 0 && isFirstRoot && (
          <div className="mt-2 flex items-center gap-2 p-2 bg-gray-50 rounded-md">
            <input
              ...
              className="w-4 h-4 text-blue-500 cursor-pointer"
            />
            <span className="font-medium text-purple-600">Custom Menu</span>
          </div>
        )}
      </>
    );
